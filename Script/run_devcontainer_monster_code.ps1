...
.EXAMPLE
    .\Script\run_devcontainer_monster_code.ps1 -Backend docker
    Uses Docker as the container backend.

.EXAMPLE
    .\Script\run_devcontainer_monster_code.ps1 -Backend podman
...
.NOTES
    ...
    ├── .devcontainer/
    └── Script/
        └── run_devcontainer_monster_code.ps1
#>
...
if ($Backend -eq 'podman') {
    ...
    Write-Host "Initializing Podman machine 'monsterVM'..."
    try {
        & podman machine init monsterVM
        Write-Host "Podman machine 'monsterVM' initialized or already exists."
    } catch {
    ...
    Write-Host "Starting Podman machine 'monsterVM'..."
    try {
        & podman machine start monsterVM -q
    ...
    Write-Host "Setting default Podman connection to 'monsterVM'..."
    try {
        & podman system connection default monsterVM
...
Write-Host "Executing 'monster' command and then starting zsh session inside container $($containerId)..."
try {
    & $Backend exec -it $containerId zsh -c 'monster; exec zsh'
    Write-Host "Interactive session ended."
} catch {
    $displayCommand = "$Backend exec -it $containerId zsh -c 'monster; exec zsh'"
