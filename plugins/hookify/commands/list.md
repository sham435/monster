...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".monster/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .monster/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.monster/hookify.my-rule.local.md` files
...
Check `${MONSTER_PLUGIN_ROOT}/examples/` for example rule files.
```
