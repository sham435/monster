...
```
{
  ...
  "prompt": "You are analyzing a Monster Code conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing Monster's actions)
...
For each confirmed behavior, create a `.monster/hookify.{rule-name}.local.md` file:
...
{Message to show Monster when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.monster/` folder, NOT the plugin directory.

Use the current working directory (where Monster Code was started) as the base path.

1. Check if `.monster/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .monster`

2. Use Write tool to create each `.monster/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.monster/hookify.{name}.local.md`
   - The path should resolve to the project's .monster directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .monster/hookify.dangerous-rm.local.md
   - .monster/hookify.console-log.local.md
   - .monster/hookify.sensitive-files.local.md
...
4. Create `.monster/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.monster/` directory (current working directory), NOT the plugin's .monster/
...
2. Ensure `.monster/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.monster/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.monster/` not plugin `.monster/`
