...
When Monster Code starts:

1. **Scan enabled plugins**: Read `.monster-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to Monster Code
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Monster Code initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Monster Code looks up → Executes
**Agents**: Task arrives → Monster Code evaluates capabilities → Selects agent
**Skills**: Task context matches description → Monster Code loads skill
**Hooks**: Event occurs → Monster Code calls matching hooks
...
**Note**: Monster Code doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Monster Code doesn't support file references.
...
```bash
#!/bin/bash
source "${MONSTER_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
