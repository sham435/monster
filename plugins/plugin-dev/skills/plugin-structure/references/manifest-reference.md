...
**Required path**: `.monster-plugin/plugin.json`

The manifest MUST be in the `.monster-plugin/` directory at the plugin root. Monster Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in Monster Code
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${MONSTER_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
When Monster Code loads components:
...
Monster Code validates the manifest on plugin load:
