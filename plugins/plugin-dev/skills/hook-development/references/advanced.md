  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MONSTER_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".monster-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .monster-hooks-config.json)
...
**Example .monster-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.monster/audit.log
...
export MONSTER_PLUGIN_ROOT="$(pwd)"
