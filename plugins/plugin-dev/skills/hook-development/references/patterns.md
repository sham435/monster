...
This reference provides common, proven patterns for implementing Monster Code hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MONSTER_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MONSTER_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
**Note:** Must restart Monster Code after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$CLAUDE_PROJECT_DIR/.monster/my-plugin.local.json"
...
**Configuration file (.monster/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
