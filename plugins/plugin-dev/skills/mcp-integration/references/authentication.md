...
Complete guide to authentication methods for MCP servers in Monster Code plugins.
...
Monster Code automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. Monster Code detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by Monster Code
...
No additional auth configuration needed! Monster Code handles everything.
...
Tokens are stored securely by Monster Code:
...
```json
{
  "api": {
    ...
    "headersHelper": "${MONSTER_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${MONSTER_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${MONSTER_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
