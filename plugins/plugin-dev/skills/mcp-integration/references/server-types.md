...
Complete reference for all MCP server types supported in Monster Code plugins.
...
{
  "my-server": {
    "command": "${MONSTER_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: Monster Code spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Monster Code session
4. **Shutdown**: Process terminated when Monster Code exits
...
{
  "custom": {
    "command": "${MONSTER_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${MONSTER_PLUGIN_ROOT}**
...
Monster Code handles OAuth flow:
