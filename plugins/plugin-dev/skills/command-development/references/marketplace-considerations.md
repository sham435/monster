...
if [ ! -f ".monster/command-initialized" ]; then
...
Checking for user config: .monster/plugin-name.local.md

if [ -f ".monster/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .monster/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .monster/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .monster/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .monster/plugin-name.local.md to customize"
fi
...
Or set in .monster/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".monster/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .monster/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .monster/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .monster/operation-completed.flag
```
