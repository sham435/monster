...
This reference covers features and patterns specific to commands bundled in Monster Code plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [MONSTER_PLUGIN_ROOT Environment Variable](#monster_plugin_root-environment-variable)
...
Monster Code automatically discovers commands in plugins using the following locations:
...
## MONSTER_PLUGIN_ROOT Environment Variable

### Purpose

`${MONSTER_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${MONSTER_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${MONSTER_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${MONSTER_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${MONSTER_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${MONSTER_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${MONSTER_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${MONSTER_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${MONSTER_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${MONSTER_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${MONSTER_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${MONSTER_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${MONSTER_PLUGIN_ROOT}`
...
Load configuration: @${MONSTER_PLUGIN_ROOT}/deploy-config.json
...
Template: @${MONSTER_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${MONSTER_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${MONSTER_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${MONSTER_PLUGIN_ROOT}/cache && date > ${MONSTER_PLUGIN_ROOT}/cache/last-run.txt`
...
- Monster will automatically use Task tool to launch agent
...
- Mention skill by name to hint Monster should invoke it
...
- Config exists: !`test -f ${MONSTER_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${MONSTER_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${MONSTER_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${MONSTER_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${MONSTER_PLUGIN_ROOT} for all plugin-internal paths**
