...
Deployment state saved to `.monster/deployment-state.local.md`:
...
Reading deployment state from `.monster/deployment-state.local.md`...
...
```markdown
.monster/plugin-name-workflow.local.md:
...
Reading workflow state from .monster/plugin-name-workflow.local.md...

Current stage: @.monster/plugin-name-workflow.local.md
...
State file: @.monster/plugin-name-workflow.local.md
...
Creating: .monster/feature-complete.flag
...
if [ -f .monster/feature-complete.flag ]; then
...
if [ -f .monster/deployment.lock ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .monster/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .monster/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .monster/deployment-checkpoints.log
...
Written to .monster/deployment-state.local.md
...
Reading state: @.monster/deployment-state.local.md
...
rm .monster/deployment-state.local.md
