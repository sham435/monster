...
head -n 20 .monster/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .monster/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .monster/commands/*.md

# Check file is in correct location
test -f .monster/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.monster/debug-logs/latest
...
cat > .monster/commands/test-bash.md << 'EOF'
---
...
cat > .monster/commands/test-forbidden.md << 'EOF'
---
...
# Command: .monster/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.monster/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .monster/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .monster/commands/; then
...
```bash
# Monitor Monster Code during command execution
# In terminal 1:
monster --debug

# In terminal 2:
watch -n 1 'ps aux | grep monster'
...
```bash
# Check file location
ls -la .monster/commands/my-command.md

# Check permissions
chmod 644 .monster/commands/my-command.md

# Check syntax
head -n 20 .monster/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .monster/commands/my-command.md
grep '\$ARGUMENTS' .monster/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .monster/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .monster/commands/my-command.md

# Verify command syntax
grep '!\`' .monster/commands/my-command.md
