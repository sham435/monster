...
Comprehensive guidance on creating Monster Code slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${MONSTER_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${MONSTER_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
Monster Code activates this skill when users:
...
Monster loads references and examples as needed based on task.
...
- **Project**: `.monster/commands/` (shared with team)
- **Personal**: `~/.monster/commands/` (your commands)
...
- ✓ Plugin command patterns (${MONSTER_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current Monster Code
