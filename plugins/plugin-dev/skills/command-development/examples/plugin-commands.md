...
Practical examples of commands designed for Monster Code plugins, demonstrating plugin-specific patterns and features.
...
!`node ${MONSTER_PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${MONSTER_PLUGIN_ROOT}` for portable path
...
**Security scan:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
Template structure: @${MONSTER_PLUGIN_ROOT}/templates/api-documentation.md
...
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${MONSTER_PLUGIN_ROOT}/scripts/package.sh $1`
...
Deployment configuration for $1: @${MONSTER_PLUGIN_ROOT}/config/$1-deploy.json
...
- Plugin's linting rules: ${MONSTER_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${MONSTER_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${MONSTER_PLUGIN_ROOT}/docs/performance.md
...
Run plugin analyzer: !`node ${MONSTER_PLUGIN_ROOT}/scripts/analyze.js $1`
...
Template: @${MONSTER_PLUGIN_ROOT}/templates/review-report.md
...
Check build script exists: !`test -x ${MONSTER_PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${MONSTER_PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${MONSTER_PLUGIN_ROOT}/config/$1.json

**Execute build:** !`bash ${MONSTER_PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${MONSTER_PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${MONSTER_PLUGIN_ROOT}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${MONSTER_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${MONSTER_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   monster /command-name args
   ```

2. **Verify ${MONSTER_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${MONSTER_PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && monster /command-name
   cd /other/project && monster /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ${MONSTER_PLUGIN_ROOT}/scripts/`
   !`ls -la ${MONSTER_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${MONSTER_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${MONSTER_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: Bash(*)
   ---
   !`bash ${MONSTER_PLUGIN_ROOT}/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.monster/plugins/my-plugin/config.json

   # Correct - works everywhere
   @${MONSTER_PLUGIN_ROOT}/config.json
   ```
