...
Detailed analysis of how production plugins use the `.monster/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".monster/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.monster/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".monster/multi-agent-swarm.local.md" > temp.md
mv temp.md ".monster/multi-agent-swarm.local.md"
```
...
Document any changes needed in MONSTER.md.
```
...
**Purpose:** Prevent session exit and loop Monster's output back as input
...
RALPH_STATE_FILE=".monster/ralph-loop.local.md"
...
# Create state file
cat > ".monster/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .monster/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.monster/multi-agent-swarm.local.md` | `.monster/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.monster/my-plugin.local.md"

# GOOD
FILE=".monster/my-plugin.local.md"
```
...
The `.monster/plugin-name.local.md` pattern provides:
