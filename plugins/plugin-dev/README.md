...
A comprehensive toolkit for developing Monster Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Monster Code plugins:
...
4. **Plugin Settings** - Configuration patterns using .monster/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in Monster Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${MONSTER_PLUGIN_ROOT}", "block dangerous commands"
...
- ${MONSTER_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${MONSTER_PLUGIN_ROOT}, user vars)
...
- ${MONSTER_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .monster/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using Monster Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Monster Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Monster Code plugins
...
Install from monster-code-marketplace:

```bash
/plugin install plugin-dev@monster-code-marketplace
```
...
This keeps Monster Code's context focused while providing deep knowledge when needed.
...
- Based on official Monster Code documentation
...
✅ **Portability**
- Use ${MONSTER_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the monster-code-marketplace. To contribute improvements:
