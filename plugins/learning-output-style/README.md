...
**Note:** This plugin differs from the original unshipped Learning output style by also incorporating all functionality from the [explanatory-output-style plugin](https://github.com/anthropics/monster-code/tree/main/plugins/explanatory-output-style), providing both interactive learning and educational insights.
...
When enabled, this plugin automatically adds instructions at the start of each session that encourage Monster to:
...
Instead of implementing everything automatically, Monster will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs Monster to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When Monster requests contributions

Monster will ask you to write code for:
...
## When Monster won't request contributions

Monster will implement directly:
...
**Monster:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, Monster will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to MONSTER.md, but it is more flexible and allows for distribution through plugins.
...
- Update the plugin - create a local copy of this plugin to personalize it
  - Hint: Ask Monster to read https://docs.monster.com/en/docs/monster-code/plugins.md and set it up for you!

## Philosophy

Learning by doing is more effective than passive observation. This plugin transforms your interaction with Monster from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
