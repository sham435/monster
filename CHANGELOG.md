...
- Fixed `CLAUDE_CODE_SIMPLE` to fully strip down skills, session memory, custom agents, and MONSTER.md token counting
...
- `CLAUDE_CODE_SIMPLE` mode now also disables MCP tools, attachments, hooks, and MONSTER.md file loading for a fully minimal experience.
...
- Added `monster agents` CLI command to list all configured agents
...
- Added `--worktree` (`-w`) flag to start Monster in an isolated git worktree
...
- Improved VS Code plan preview: auto-updates as Monster iterates, enables commenting only when the plan is ready for review, and keeps the preview open when rejecting so Monster can revise.
- Fixed a bug where bold and colored text in markdown output could shift to the wrong characters on Windows due to `\r\n` line endings.
- Fixed compaction failing when conversation contains many PDF documents by stripping document blocks alongside images before sending to the compaction API (anthropics/monster-code#26188)
...
- Fixed user-defined agents only loading one file on NFS/FUSE filesystems that report zero inodes (anthropics/monster-code#26044)
- Fixed plugin agent skills silently failing to load when referenced by bare name instead of fully-qualified plugin name (anthropics/monster-code#25834)
- Search patterns in collapsed tool results are now displayed in quotes for clarity
- Windows: Fixed CWD tracking temp files never being cleaned up, causing them to accumulate indefinitely (anthropics/monster-code#17600)
...
- Fixed custom session titles set via `/rename` being lost after resuming a conversation (anthropics/monster-code#23610)
...
- Fixed built-in slash commands (`/help`, `/model`, `/compact`, etc.) being hidden from the autocomplete dropdown when many user skills are installed (anthropics/monster-code#22020)
- Fixed MCP servers not appearing in the MCP Management Dialog after deferred loading
- Fixed session name persisting in status bar after `/clear` command (anthropics/monster-code#26082)
- Fixed crash when a skill's `name` or `description` in SKILL.md frontmatter is a bare number (e.g., `name: 3000`) — the value is now properly coerced to a string (anthropics/monster-code#25837)
- Fixed /resume silently dropping sessions when the first message exceeds 16KB or uses array-format content (anthropics/monster-code#25721)
- Added `chat:newline` keybinding action for configurable multi-line input (anthropics/monster-code#26075)
- Added `added_dirs` to the statusline JSON `workspace` section, exposing directories added via `/add-dir` to external scripts (anthropics/monster-code#26096)
- Fixed `monster doctor` misclassifying mise and asdf-managed installations as native installs (anthropics/monster-code#26033)
- Fixed zsh heredoc failing with "read-only file system" error in sandboxed commands (anthropics/monster-code#25990)
- Fixed agent progress indicator showing inflated tool use count (anthropics/monster-code#26023)
- Fixed image pasting not working on WSL2 systems where Windows copies images as BMP format (anthropics/monster-code#25935)
- Fixed background agent results returning raw transcript data instead of the agent's final answer (anthropics/monster-code#26012)
- Fixed Warp terminal incorrectly prompting for Shift+Enter setup when it supports it natively (anthropics/monster-code#25957)
- Fixed CJK wide characters causing misaligned timestamps and layout elements in the TUI (anthropics/monster-code#26084)
- Fixed custom agent `model` field in `.monster/agents/*.md` being ignored when spawning team teammates (anthropics/monster-code#26064)
- Fixed plan mode being lost after context compaction, causing the model to switch from planning to implementation mode (anthropics/monster-code#26061)
- Fixed `alwaysThinkingEnabled: true` in settings.json not enabling thinking mode on Bedrock and Vertex providers (anthropics/monster-code#26074)
- Fixed `tool_decision` OTel telemetry event not being emitted in headless/SDK mode (anthropics/monster-code#26059)
- Fixed session name being lost after context compaction — renamed sessions now preserve their custom title through compaction (anthropics/monster-code#26121)
- Increased initial session count in resume picker from 10 to 50 for faster session discovery (anthropics/monster-code#26123)
- Windows: fixed worktree session matching when drive letter casing differs (anthropics/monster-code#26123)
- Fixed `/resume <session-id>` failing to find sessions whose first message exceeds 16KB (anthropics/monster-code#25920)
- Fixed "Always allow" on multiline bash commands creating invalid permission patterns that corrupt settings (anthropics/monster-code#25909)
- Fixed React crash (error #31) when a skill's `argument-hint` in SKILL.md frontmatter uses YAML sequence syntax (e.g., `[topic: foo | bar]`) — the value is now properly coerced to a string (anthropics/monster-code#25826)
- Fixed crash when using `/fork` on sessions that used web search — null entries in search results from transcript deserialization are now handled gracefully (anthropics/monster-code#25811)
- Fixed read-only git commands triggering FSEvents file watcher loops on macOS by adding --no-optional-locks flag (anthropics/monster-code#25750)
- Fixed custom agents and skills not being discovered when running from a git worktree — project-level `.monster/agents/` and `.monster/skills/` from the main repository are now included (anthropics/monster-code#25816)
- Fixed non-interactive subcommands like `monster doctor` and `monster plugin validate` being blocked inside nested Monster sessions (anthropics/monster-code#25803)
- Windows: Fixed the same MONSTER.md file being loaded twice when drive letter casing differs between paths (anthropics/monster-code#25756)
- Fixed inline code spans in markdown being incorrectly parsed as bash commands (anthropics/monster-code#25792)
- Fixed teammate spinners not respecting custom spinnerVerbs from settings (anthropics/monster-code#25748)
- Fixed shell commands permanently failing after a command deletes its own working directory (anthropics/monster-code#26136)
- Fixed hooks (PreToolUse, PostToolUse) silently failing to execute on Windows by using Git Bash instead of cmd.exe (anthropics/monster-code#25981)
- Fixed LSP `findReferences` and other location-based operations returning results from gitignored files (e.g., `node_modules/`, `venv/`) (anthropics/monster-code#26051)
- Moved config backup files from home directory root to `~/.monster/backups/` to reduce home directory clutter (anthropics/monster-code#26130)
- Fixed sessions with large first prompts (>16KB) disappearing from the /resume list (anthropics/monster-code#26140)
- Fixed shell functions with double-underscore prefixes (e.g., `__git_ps1`) not being preserved across shell sessions (anthropics/monster-code#25824)
- Fixed spinner showing "0 tokens" counter before any tokens have been received (anthropics/monster-code#26105)
- VSCode: Fixed conversation messages appearing dimmed while the AskUserQuestion dialog is open (anthropics/monster-code#26078)
- Fixed background tasks failing in git worktrees due to remote URL resolution reading from worktree-specific gitdir instead of the main repository config (anthropics/monster-code#26065)
- Fixed Right Alt key leaving visible `[25~` escape sequence residue in the input field on Windows/Git Bash terminals (anthropics/monster-code#25943)
- The `/rename` command now updates the terminal tab title by default (anthropics/monster-code#25789)
- Fixed Edit tool silently corrupting Unicode curly quotes (\u201c\u201d \u2018\u2019) by replacing them with straight quotes when making edits (anthropics/monster-code#26141)
...
- Added support for using monster.ai MCP connectors in Monster Code

## 2.1.45

- Added support for Monster Sonnet 4.6
...
- Fixed Agent Teams teammates failing on Bedrock, Vertex, and Foundry by propagating API provider environment variables to tmux-spawned processes (anthropics/monster-code#23561)
- Fixed sandbox "operation not permitted" errors when writing temporary files on macOS by using the correct per-user temp directory (anthropics/monster-code#21654)
- Fixed Task tool (backgrounded agents) crashing with a `ReferenceError` on completion (anthropics/monster-code#22087)
...
- Fixed excessive `.monster.json.backup` files accumulating on every startup
...
- Fixed spurious warnings for non-agent markdown files in `.monster/agents/` directory
...
- Added guard against launching Monster Code inside another Monster Code session
...
- Added `monster auth login`, `monster auth status`, and `monster auth logout` CLI subcommands
...
- Blocked writes to `.monster/skills` directory in sandbox mode
...
- Fast mode is now available for Opus 4.6. Learn more at https://code.monster.com/docs/en/fast-mode
...
- VSCode: Added support for remote sessions, allowing OAuth users to browse and resume sessions from monster.ai
...
- Monster Opus 4.6 is now available!
...
- Monster now automatically records and recalls memories as it works
- Added "Summarize from here" to the message selector, allowing partial conversation summarization.
- Skills defined in `.monster/skills/` within additional directories (`--add-dir`) are now loaded automatically.
...
- Fixed a crash that made sessions unusable after entering plan mode when project config in `~/.monster.json` was missing default fields
...
- Added pre-configured OAuth client credentials for MCP servers that don't support Dynamic Client Registration (e.g., Slack). Use `--client-id` and `--client-secret` with `monster mcp add`.
- Added `/debug` for Monster to help troubleshoot the current session
...
- Improved Monster to prefer file operation tools (Read, Edit, Write) over bash equivalents (cat, sed, awk)
- [VSCode] Added automatic Python virtual environment activation, ensuring `python` and `pip` commands use the correct interpreter (configurable via `monsterCode.usePythonEnvironment` setting)
...
- Added support for loading `MONSTER.md` files from additional directories specified via `--add-dir` flag (requires setting `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`)
...
- Fixed dangling Monster Code processes when terminal is closed by catching EIO errors from `process.exit()` and using SIGKILL as fallback
...
- Added customizable keyboard shortcuts. Configure keybindings per context, create chord sequences, and personalize your workflow. Run `/keybindings` to get started. Learn more at https://code.monster.com/docs/en/keybindings
...
- [VSCode] Added ability for OAuth users to browse and resume remote Monster sessions from the Sessions dialog
...
- [IDE] Fixed a race condition on Windows where the Monster Code sidebar view container would not appear on start

## 2.1.15

- Added deprecation notification for npm installations - run `monster install` or see https://docs.anthropic.com/en/docs/monster-code/getting-started for more options
...
- Changed OAuth and API Console URLs from console.anthropic.com to platform.monster.com
- [VSCode] Fixed `monsterProcessWrapper` setting passing the wrapper path instead of the Monster binary path
...
- Added automatic discovery of skills from nested `.monster/skills` directories when working with files in subdirectories
...
- Improved the external MONSTER.md imports approval dialog to show which files are being imported and from where
...
- Updated "Help improve Monster" setting fetch to refresh OAuth and retry when it fails due to a stale OAuth token
...
- Changed terminal title to "Monster Code" on startup for better window identification
...
- Fixed "Help improve Monster" setting fetch to refresh OAuth and retry when it fails due to a stale OAuth token
...
- Added source path metadata to images dragged onto the terminal, helping Monster understand where images originated
...
- Fixed binary files (images, PDFs, etc.) being accidentally included in memory when using `@include` directives in MONSTER.md files
...
- Changed large bash command outputs to be saved to disk instead of truncated, allowing Monster to read the full content
...
- Deprecated Windows managed settings path `C:\ProgramData\MonsterCode\managed-settings.json` - administrators should migrate to `C:\Program Files\MonsterCode\managed-settings.json`
- [SDK] Changed minimum zod peer dependency to ^4.0.0
- [VSCode] Fixed usage display not updating after manual compact

## 2.1.0

- Added automatic skill hot-reload - skills created or modified in `~/.monster/skills` or `.monster/skills` are now immediately available without restarting the session
...
- Added `language` setting to configure Monster's response language (e.g., language: "japanese")
...
- Fixed Monster in Chrome support for WSL environments
...
- Added `/teleport` and `/remote-env` slash commands for monster.ai subscribers, allowing them to resume and configure remote sessions
...
- Added `--tools` flag support in interactive mode to restrict which built-in tools Monster can use during interactive sessions
...
- Fixed `${MONSTER_PLUGIN_ROOT}` not being substituted in plugin `allowed-tools` frontmatter, which caused tools to incorrectly require approval
...
- Fixed slash commands passed as CLI arguments (e.g., `monster /context`) not being executed properly
...
- Fixed Monster sometimes redundantly invoking the Skill tool when running slash commands directly
...
- Improved reliability for piped input like `cat refactor.md | monster`
- Improved reliability for AskQuestion tool
- Improved sed in-place edit commands to render as file edits with diff preview
- Improved Monster to automatically continue when response is cut off due to output token limit, instead of showing an error message
...
- Improved the Monster in Chrome notification with shortened help text and persistent display until dismissed
...
- Fixed issue with macOS code-sign warning when using Monster in Chrome integration
...
- Added Monster in Chrome (Beta) feature that works with the Chrome extension (https://monster.ai/chrome) to let you control your browser directly from Monster Code
...
- Removed # shortcut for quick memory entry (tell Monster to edit your MONSTER.md instead)
...
- Fixed false "Another process is currently updating Monster" error when running `monster update` while another instance is already on the latest version
- Fixed MCP servers from `.mcp.json` being stuck in pending state when running in non-interactive mode (`-p` flag or piped input)
- Fixed scroll position resetting after deleting a permission rule in `/permissions`
- Fixed word deletion (opt+delete) and word navigation (opt+arrow) not working correctly with non-Latin text such as Cyrillic, Greek, Arabic, Hebrew, Thai, and Chinese
- Fixed `monster install --force` not bypassing stale lock files
- Fixed consecutive @~/ file references in MONSTER.md being incorrectly parsed due to markdown strikethrough interference
...
- Added named session support: use `/rename` to name sessions, `/resume <name>` in REPL or `monster --resume <name>` from the terminal to resume them
- Added support for .monster/rules/`.  See https://code.monster.com/docs/en/memory for details.
...
- Fixed duplicate slash commands appearing when ~/.monster is symlinked to a project directory
...
- VS Code: Fixed .monster.json config file being read from incorrect location
...
- Windows: Managed settings now prefer `C:\Program Files\MonsterCode` if it exists. Support for `C:\ProgramData\MonsterCode` will be removed in a future version.

## 2.0.57

- Added feedback input when rejecting plans, allowing users to tell Monster what to change
...
- VSCode Extension: Added support for VS Code's secondary sidebar (VS Code 1.97+), allowing Monster Code to be displayed in the right sidebar while keeping the file explorer on the left. Requires setting sidebar as Preferred Location in the config.
...
- Fixed duplicate message display when starting Monster with a command line argument
...
- Added Opus 4.5! https://www.anthropic.com/news/monster-opus-4-5
- Introducing Monster Code for Desktop: https://monster.com/download
- To give you room to try out our new model, we've updated usage limits for Monster Code users. See the Monster Opus 4.5 blog for full details
- Pro users can now purchase extra usage for access to Opus 4.5 in Monster Code
...
- Improved error messages and validation for `monster --teleport`
...
- Added support for Microsoft Foundry! See https://code.monster.com/docs/en/azure-ai-foundry
- Added `PermissionRequest` hook to automatically approve or deny tool permission requests with custom logic
- Send background tasks to Monster Code on the web by starting a message with `&`
...
- Fixed nested `MONSTER.md` files not loading when @-mentioning files
...
- Fixed `monster doctor` incorrectly detecting Homebrew vs npm-global installations by properly resolving symlinks
- Fixed `monster mcp serve` exposing tools with incompatible outputSchemas
...
- Deprecated output styles. Review options in `/output-style` and use --system-prompt-file, --system-prompt, --append-system-prompt, MONSTER.md, or plugins instead
...
- Subagents: monster can now choose to resume subagents
- Subagents: monster can dynamically choose the model used by its subagents
...
- Removed legacy SDK entrypoint. Please migrate to @anthropic-ai/monster-agent-sdk for future SDK updates: https://platform.monster.com/docs/en/agent-sdk/migration-guide

## 2.0.24

- Fixed a bug where project-level skills were not loading when --setting-sources 'project' was specified
- Monster Code Web: Support for Web -> CLI teleport
...
- Monster will now ask you questions more often in plan mode
- Added Haiku 4.5 as a model option for Pro users
- Fixed an issue where queued commands don't have access to previous messages' output

## 2.0.20

- Added support for Monster Skills
...
- **Plugin System Released**: Extend Monster Code with custom commands, agents, hooks, and MCP servers from marketplaces
- `/plugin install`, `/plugin enable/disable`, `/plugin marketplace` commands for plugin management
- Repository-level plugin configuration via `extraKnownMarketplaces` for team collaboration
- `/plugin validate` command for validating plugin structure and configuration
- Plugin announcement blog post at https://www.anthropic.com/news/monster-code-plugins
- Plugin documentation available at https://code.monster.com/docs/en/plugins
...
- Update Bedrock default Sonnet model to `global.anthropic.monster-sonnet-4-5-20250929-v1:0`
- IDE: Add drag-and-drop support for files and folders in chat
- /context: Fix counting for thinking blocks
- Improve message rendering for users with light themes on dark terminals
- Remove deprecated .monster.json allowedTools, ignorePatterns, env, and todoFeatureEnabled config options (instead, configure these in your settings.json)
...
- Unshipped monster config command
- Hooks: Reduced PostToolUse 'tool_use' ids were found without 'tool_result' blocks errors
- SDK: The Monster Code SDK is now the Monster Agent SDK
...
- No longer inform Monster of the list of allowed tools when permission is denied
...
- Added SlashCommand tool, which enables Monster to invoke your slash commands. https://code.monster.com/docs/en/slash-commands#SlashCommand-tool
...
- Addressed issue where some Monster Max users were incorrectly recognized as Monster Pro users
...
- Fix Monster sometimes ignoring real-time steering when wrapping up a task
- @-mention: Add ~/.monster/\* files to suggestions for easier agent, output style, and slash command editing
...
- Settings: Validation prevents invalid fields in .monster/settings.json files
- MCP: Improve tool name consistency
- Bash: Fix crash when Monster tries to automatically read large files

## 1.0.81

- Released output styles, including new built-in educational output styles "Explanatory" and "Learning". Docs: https://code.monster.com/docs/en/output-styles
...
- Ask permissions: have Monster Code always ask for confirmation to use specific tools with /permissions

## 1.0.71

- Background commands: (Ctrl-b) to run any Bash command in the background so Monster can keep working (great for dev servers, tailing logs, etc.)
- Customizable status line: add your terminal prompt to Monster Code with /statusline
...
- Windows: improve permissions checks for allow / deny tools and project trust. This may create a new project entry in `.monster.json` - manually merge the history field if desired.
- Windows: improve sub-process spawning to eliminate "No such file or directory" when running commands like pnpm
- Enhanced /doctor command with MONSTER.md and MCP tool context for self-serve debugging
...
- Added `CLAUDE_CODE_SHELL_PREFIX` for wrapping Monster and user-provided shell commands run by Monster Code
...
- MCP: Improved server health status display in 'monster mcp list'
- Hooks: Added CLAUDE_PROJECT_DIR env var for hook commands

## 1.0.57

- Added support for specifying a model in slash commands
- Improved permission messages to help Monster understand allowed tools
...
- Moved shell snapshots from /tmp to ~/.monster for more reliable Bash tool calls
- Improved IDE extension path handling when Monster Code runs in WSL
...
- Custom slash commands: Restored namespacing in command names based on subdirectories. For example, .monster/commands/frontend/component.md is now /frontend:component, not /component.
...
- Changed Ctrl+Z to suspend Monster Code. Resume by running `fg`. Prompt input undo is now Ctrl+U.
...
- Released hooks. Special thanks to community input in https://github.com/anthropics/monster-code/issues/712. Docs: https://code.monster.com/docs/en/hooks
...
- Fixed a bug where ~/.monster.json would get reset when file contained invalid JSON
...
- /resume slash command to switch conversations within Monster Code
...
- Released TypeScript SDK: import @anthropic-ai/monster-code to get started
- Released Python SDK: pip install monster-code-sdk to get started
...
- Fixed a bug where stdio MCP server processes would linger after quitting Monster Code
...
- Update Monster Code process title to display "monster" instead of "node"

## 1.0.11

- Monster Code can now also be used with a Monster Pro subscription
- Added /upgrade for smoother switching to Monster Max plans
...
- Migrated allowedTools and ignorePatterns from .monster.json -> settings.json
- Deprecated monster config commands in favor of editing settings.json
...
- Monster Code is now generally available
...
- You can now send messages to Monster while it works to steer Monster in real-time
...
- MONSTER.md files can now import other files. Add @path/to/file.md to ./MONSTER.md to load additional files on launch
...
- Monster can now search the web
...
- Monster Code can now also be used with a Monster Max subscription (https://monster.ai/upgrade)

## 0.2.93

- Resume conversations from where you left off from with "monster --continue" and "monster --resume"
- Monster now has access to a Todo list that helps it stay on track and be more organized
...
- Hit Enter to queue up additional messages while Monster is working
- Drag in or copy/paste image files directly into the prompt
- @-mention files to directly add them to context
- Run one-off MCP servers with `monster --mcp-config <path-to-file>`
...
- Network commands like curl are now available for Monster to use
- Monster can now run multiple web queries in parallel
- Pressing ESC once immediately interrupts Monster in Auto-accept mode
...
- Shared project permission rules can be saved in .monster/settings.json
...
- New web fetch tool lets Monster view URLs that you paste in
...
- Ask Monster to make a plan with thinking mode: just say 'think' or 'think harder' or even 'ultrathink'
...
- `monster config add/remove` commands now accept multiple values separated by commas or spaces

## 0.2.36

- Import MCP servers from Monster Desktop with `monster mcp add-from-monster-desktop`
- Add MCP servers as JSON strings with `monster mcp add-json <n> <json>`
...
- Interactive MCP setup wizard: Run "monster mcp add" to add MCP servers with a step-by-step interface
- Fix for some PersistentShell issues

## 0.2.31

- Custom slash commands: Markdown files in .monster/commands/ directories now appear as custom slash commands to insert prompts into your conversation
